...
import db_funcs_for_subjects_db_PT_undergrade
...
def return_list_of_lists_of_dates(work_sheet):
    ...
    for row in list_of_times_first_lessons:
        print(row)
        print(work_sheet.cell(row = row, column = 1))
        print(type(work_sheet.cell(row = row, column = 1)))
        print(isMerged(work_sheet, 7, 1))
        print(get_value_of_merged_call(work_sheet, 7, 1))
        print(work_sheet.cell(row = row, column = 1).value())
                ...
                if type(group_cell_value) == str and 'Группа' in group_cell_value:
                    try:
                        ...
                        if isMerged(work_sheet, row_of_first_lesson_of_day + num_of_lesson, group_column):
                            ...
                            db_funcs_for_subjects_db_PT_undergrade.save_subj(name_for_db, date, time, group_cell_value, merged_subject.replace("\n"," "))
                        elif type(subject) == str:
                            db_funcs_for_subjects_db_PT_undergrade.save_subj(name_for_db, date, time, group_cell_value, subject.replace("\n"," "))
                        elif subject == None:
                            db_funcs_for_subjects_db_PT_undergrade.save_subj(name_for_db, date, time, group_cell_value, 'Нет предмета')
                    except:
...
def return_db_name(file_name):
    if 'part_time_1_undergraduate' in file_name:
        return 'part_time_1_undergraduate'
    elif 'part_time_2_undergraduate' in file_name:
        return 'part_time_2_undergraduate'
    elif 'part_time_3_undergraduate' in file_name:
        return 'part_time_3_undergraduate'
    elif 'part_time_4_undergraduate' in file_name:
        return 'part_time_4_undergraduate'
...
def create_groups_in_db(work_book, work_sheet_for_create_groups, db_name):
    ...
    db_funcs_for_subjects_db_PT_undergrade.save_groups(db_name, list_of_groups)
    
def pars_files_create_dbfiles():
    work_files = glob.glob('time_tables/part_time_undergraduate/*.xlsx')
    print(work_files)
    for work_file in work_files:
        print(work_file)
        db_name = return_db_name(work_file)
        db_funcs_for_subjects_db_PT_undergrade.create_db(db_name)
        ...
        for work_sheet in work_book.sheetnames:
            ...
            db_funcs_for_subjects_db_PT_undergrade.save_dates_and_times(db_name, list_of_dates, list_of_times)
