            ...
            if self.is_merged(worksheet, viewed_group_cell) == False:
                ...
                try:
                    ...
                    if viewed_group_value != const_merged_group:
                        raise File_not_valid(f'Объединённая ячейка в {viewed_group_cell.coordinate} в листе {worksheet_name}')
                except:
                    raise File_not_valid(f'Объединённая ячейка в {viewed_group_cell.coordinate} в листе {worksheet_name}')
            viewed_group_cell = worksheet.cell(row = const_group_specialization_row, column = column)
            if const_group_specializations[index_of_group_name_and_specialization] != viewed_group_cell.value:
                raise File_not_valid(f'Ошибка в специализации в {viewed_group_cell.coordinate} в листе {worksheet_name}')
