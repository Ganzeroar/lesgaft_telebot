...
def save_groups(db_name, list_of_groups):
    ...
    for group in list_of_groups:
        cursor.execute("ALTER TABLE " + db_name +
                       " ADD COLUMN " + str(group) + " text")
...
def save_subj(db_name, date, time, group, subj):
    ...
    string_sql = "UPDATE " + db_name + " SET " + group + " = '" + \
        str(subj) + "' WHERE date = '" + str(date) + \
        "' AND time = '" + str(time) + "'"
...
def get_subjects_today(name_of_group, db_name, date):
    ...
    if name_of_group in columns_names:
        req = "SELECT " + str(name_of_group) + " FROM " + \
            str(db_name) + " WHERE date = '" + str(date) + "'"
        cursor.execute(req)
