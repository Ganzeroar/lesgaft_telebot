...
class Test_check_cells_with_lessons(unittest.TestCase):
    ...
    def test_take_correct_imist_1_cells_with_lessons_return_correct_text(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_4.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
    ...
    def test_take_incorrect_imist_1_lesson_cell_with_new_line_return_exception(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_5.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
        with self.assertRaises(excel_validator_imist.File_not_valid) as context:
            obj.check_cells_with_lessons(work_book, work_file_name)
        self.assertEqual('Ошибка в ячейке в G7 в листе 11.04. - 16.04. в предмете "[\'Философия\', \'Практическое занятие\', \'\']"', str(context.exception))

    def test_take_incorrect_imist_1_lesson_cell_with_not_existing_subj_return_exception(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_6.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
        with self.assertRaises(excel_validator_imist.File_not_valid) as context:
            obj.check_cells_with_lessons(work_book, work_file_name)
        self.assertEqual('Ошибка в ячейке в G8 в листе 11.04. - 16.04. в предмете "[\'Делавые коммуникации\', \'Лекция\']"', str(context.exception))
    
    def test_take_incorrect_imist_1_lesson_cell_with_not_existing_subj_type_return_exception(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_7.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
        with self.assertRaises(excel_validator_imist.File_not_valid) as context:
            obj.check_cells_with_lessons(work_book, work_file_name)
        self.assertEqual('Ошибка в ячейке в D7 в листе 11.04. - 16.04. в предмете "[\'Философия\', \'Практическое занятее\']"', str(context.exception))
    
    def test_take_incorrect_imist_1_location_cell_with_not_existing_location_return_exception(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_8.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
        with self.assertRaises(excel_validator_imist.File_not_valid) as context:
            obj.check_cells_with_lessons(work_book, work_file_name)
        self.assertEqual('Ошибка в ячейке в E18 в листе 11.04. - 16.04. в локации "ауд. 500"', str(context.exception))

    def test_take_incorrect_imist_1_teacher_cell_with_not_existing_teacher_return_exception(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_9.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
        with self.assertRaises(excel_validator_imist.File_not_valid) as context:
            obj.check_cells_with_lessons(work_book, work_file_name)
        self.assertEqual('Ошибка в ячейке в F6 в листе 11.04. - 16.04. в преподавателе [\'ВВВВВВВ\']', str(context.exception))

    def test_take_incorrect_imist_1_second_teacher_cell_with_not_existing_teacher_return_exception(self):
        work_file = glob.glob(f'test_time_tables/full_time_undergraduate_imist/1_imist_10.xlsx')
        work_file_name = work_file[0]
        work_book = load_workbook(work_file_name)
        obj = excel_validator_imist.Excel_validator_imist()
        with self.assertRaises(excel_validator_imist.File_not_valid) as context:
            obj.check_cells_with_lessons(work_book, work_file_name)
        self.assertEqual('Ошибка в ячейке в I11 в листе 11.04. - 16.04. в преподавателе [\'Ермилова В.В.\', \'В.А.\']', str(context.exception))
